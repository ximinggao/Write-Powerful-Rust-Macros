...
use quote::quote;
use syn::{Ident, LitInt, Token, parse::Parse, parse_macro_input, spanned::Spanned};
    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        while !input.is_empty() && !input.peek(kw::bucket) {
            if input.peek(kw::mem) {
                let _ = input
                    .parse::<kw::mem>()
                    .expect("we just checked for the mem token");
                ...
                let _ = input
                    .parse::<kw::time>()
                    .expect("we just checked for the time token");
